...
Shape::Shape(const std::string & fileName, const std::string & textureFileName, const unsigned maxFaces)
{
	...
	//create cube mesh 
	Vertex * cubeVertices = init_cube(mesh->myKdtree.getRoot()->center.x, mesh->myKdtree.getRoot()->center.y, mesh->myKdtree.getRoot()->center.z, mesh->myKdtree.getRoot()->halfWidth, mesh->myKdtree.getRoot()->halfHeight, mesh->myKdtree.getRoot()->halfDepth);
