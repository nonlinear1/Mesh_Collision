...
Scene::Scene(glm::vec3 position,float angle,float hwRelation,float near, float far)
{
	...
	void Scene::draw(int shaderIndx,int cameraIndx,bool drawAxis,int index,Node* smallest)
	{
		...
		for (int i=0; i<shapes.size();i++)
		{
			...
			if (i == index) {
				if (smallest != nullptr) {
					Vertex * cubeVertices = shapes[index]->init_cube(smallest->center.x, smallest->center.y, smallest->center.z, smallest->halfWidth, smallest->halfHeight, smallest->halfDepth);
